...
const AppState = (() => {
    ...
    const freshSeed = () => SEED_PRODUCTS.map(p => ({ id: Format.uid(), ...p }));
    ...
    const persistStore = () => Storage.save(Storage.KEY.store, storeInfo);
    const cartSubtotal = () => state.cart.reduce((s, i) => s + i.price * i.qty, 0);
    const getNominalDiscount = () => {
        const v = state.discount.value || 0;
        return state.discount.type === '%' ? Math.floor(cartSubtotal() * (v / 100)) : v;
    };
    const cartTotal = () => Math.max(0, cartSubtotal() - getNominalDiscount());
    const cartCount = () => state.cart.reduce((s, i) => s + i.qty, 0);
    const cartQty = id => (state.cart.find(x => x.id === id) || {}).qty || 0;
    return { state, storeInfo, freshSeed, nextTxNo, persist, persistStore, cartSubtotal, getNominalDiscount, cartTotal, cartCount, cartQty };
})();
