...
function renderVariantRows() {
    document.getElementById('variantRows').innerHTML = _editVariants.map((v, i) =>
        `<div style="display:flex;gap:6px;margin-bottom:6px;align-items:center">
                <input class="field__input" style="flex:2" placeholder="Nama varian (mis: L, Pedas)" value="${esc(v.name)}" data-vi="${i}" data-vf="name" />
...
export const ProductModal = {
    mount() {
        ...
        document.getElementById('addVariantBtn').addEventListener('click', () => { _editVariants.push({ id: uid(), name: '', price: null }); renderVariantRows(); });
