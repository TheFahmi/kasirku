...
const ProductGrid = (() => {
    ...
    const render = () => {
        ...
        renderStockAlert();
        if (!list.length) {
            grid.innerHTML = `<div style="grid-column:1/-1">${Swatch.emptyState('Tidak ditemukan', 'Coba kata kunci atau kategori lain')}</div>`;
            return;
        }
        ...
        setTimeout(() => {
            grid.innerHTML = list.map(p => {
                ...
                const [bg, fg] = Swatch.swatch(p.category);
  ...
  <div class="pcard__media" style="--tile-bg:${bg};--tile-fg:${fg}">
    <span class="pcard__cat">${Format.esc(p.category)}</span>${Format.esc(Format.initials(p.name))}
  ...
  <div class="pcard__body">
    <div class="pcard__name">${Format.esc(p.name)}${hasVar ? ' <small style="color:var(--muted);font-size:10px">▾ varian</small>' : ''}</div>
