...
const PaymentModal = (() => {
    ...
    function setPayMethod(m) {
        ...
        if (isCash) {
            ...
            document.getElementById('nonCashAmount').textContent = Format.formatRupiah(AppState.cartTotal());
    ...
    function updateChange() {
        ...
        document.getElementById('paymentChange').textContent        = Format.formatRupiah(Math.max(0, c));
        document.getElementById('completePaymentBtn').disabled      = c < 0;
    }

    function openPayment() {
        const t = AppState.cartTotal();
        document.getElementById('paymentTotal').textContent = Format.formatRupiah(t);
        document.getElementById('cashInput').value          = '';
        document.getElementById('paymentChange').textContent = Format.formatRupiah(0);
        document.getElementById('quickCash').innerHTML = quickCash(t)
            .map(v => `<button data-cash="${v}">${v === t ? 'Uang pas' : Format.formatRupiah(v)}</button>`).join('');
        document.getElementById('payMethods').innerHTML = getPayMethods()
            .map(m => `<button type="button" class="paymethod" data-method="${m.id}">${methodIcon(m)}<span>${Format.esc(m.label)}</span></button>`).join('');
        setPayMethod('cash');
        Modal.closeModal('cartSheet');
        Modal.openModal('paymentModal');
    ...
    function showOrderConfirm() {
        ...
        const rows = AppState.state.cart.map(i =>
            `<div class="ritem"><div class="ritem__name">${Format.esc(i.name)}</div>
             <div class="ritem__qty">${i.qty} × ${Format.formatRupiah(i.price)}</div>
             <div class="ritem__sub">${Format.formatRupiah(i.price * i.qty)}</div></div>`
        ).join('');
        const cash = parseFloat(document.getElementById('cashInput').value) || 0;
        document.getElementById('orderConfirmContent').innerHTML = `
            <div class="rule" style="margin-top:0"></div>${rows}<div class="rule"></div>
            ${AppState.getNominalDiscount() ? `<div class="rline"><span>Diskon</span><span>-${Format.formatRupiah(AppState.getNominalDiscount())}</span></div>` : ''}
            <div class="rline rline--total"><span>TOTAL</span><span>${Format.formatRupiah(total)}</span></div>
            <div class="rule"></div>
            <div class="rline"><span>Metode</span><span>${getPayLabel(_payMethod)}</span></div>
            ${isCash ? `<div class="rline"><span>Uang Diterima</span><span>${Format.formatRupiah(cash)}</span></div>
            <div class="rline"><span>Kembalian</span><span style="color:var(--success);font-weight:700">${Format.formatRupiah(cash - total)}</span></div>` : ''}
            ${document.getElementById('orderNote').value.trim() ? `<div class="rule"></div><div class="rline"><span>Catatan</span><span style="color:var(--accent-ink)">${Format.esc(document.getElementById('orderNote').value.trim())}</span></div>` : ''}`;
        Modal.openModal('orderConfirmModal');
    }
    ...
    function completeTx() {
        ...
        const tx = {
            id: Format.uid(), no: AppState.nextTxNo(), date: new Date().toISOString(), cashier, note,
        ...
        Modal.closeModal('orderConfirmModal');
        Modal.closeModal('paymentModal');
