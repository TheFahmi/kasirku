...
import { esc, uid } from '../utils/format.js';
...
function renderMethodList() {
    ...
    document.getElementById('methodList').innerHTML = `<div class="panel">${methods.map(m => `<div class="mmethod"><div class="mmethod__label">${esc(m.label)}</div><span class="mmethod__type mmethod__type--${m.type}">${m.type === 'cash' ? 'Tunai' : 'Non-Tunai'}</span>${m.builtin ? '' : `<button class="mmethod__del" data-del-method="${m.id}" aria-label="Hapus">✕</button>` }</div>`).join('')}</div>`;
}
...
const PayMethodsModal = {
    mount() {
        ...
        document.getElementById('addMethodForm').addEventListener('submit', e => {
            ...
            const type = document.querySelector('input[name="newMethodType"]:checked').value; const id = 'custom_' + uid();
